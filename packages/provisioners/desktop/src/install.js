...
const CACHE_DIR = join(homedir(), `.${ORG}`, `.${PROJECT}`, 'desktop');
...
function fetchJson(url, _depth = 0) {
  return new Promise((resolve, reject) => {
    ...
    get(url, { headers: { 'User-Agent': 'create-lamaste-desktop' } }, (res) => {
...
async function installLinux(appImagePath) {
  ...
  const dest = join(binDir, 'lamaste-desktop');
  execSync(`cp "${appImagePath}" "${dest}"`);
  chmodSync(dest, 0o755);

  console.log(`  Installed to ${dest}`);
  console.log(`  Make sure ${binDir} is in your PATH, then run: lamaste-desktop`);
  console.log('  Launching...');
  spawn(dest, [], { detached: true, stdio: 'ignore' }).unref();
}

export async function install() {
  console.log('\n  Lamaste Desktop Installer\n');
