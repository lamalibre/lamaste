...
export async function detectOS() {
  ...
  try {
    ...
    throw new Error('Could not read /etc/os-release. Lamaste requires Ubuntu 24.04.');
  }
  ...
  if (id !== 'ubuntu' || !versionId.startsWith('24.04')) {
    throw new Error(`Lamaste requires Ubuntu 24.04. Detected: ${prettyName}`);
  }
...
export async function detectIP({ allowPrivate = false } = {}) {
  ...
  throw new Error(
    'Could not detect a public IP address. Lamaste must be installed on a server with a public IP.',
  );
}
...
export function checkRoot() {
  if (process.getuid() !== 0) {
    throw new Error(
      'Lamaste installer must be run as root. Try: sudo npx @lamalibre/create-lamaste',
    );
  }
}
