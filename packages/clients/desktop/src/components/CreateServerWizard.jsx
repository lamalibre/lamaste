...
const DOCS_BASE = 'https://lamalibre.github.io/lamaste';

const DISCLAIMER_KEY = 'lamalibre.cloud.disclaimer-dismissed';
...
function SizeStep({ sizes, selectedSize, setSelectedSize, loading, error }) {
  ...
  return (
    <div className="space-y-2">
      <p className="text-xs text-zinc-400">
        Select a droplet size. The minimum size is sufficient for most Lamaste deployments.
      </p>
    ...
    <div className="space-y-3">
      <div>
        ...
        <input
          ...
          placeholder={`lamaste-${region || 'nyc1'}`}
          className="w-full bg-zinc-950 border border-zinc-700 rounded px-3 py-2 text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-cyan-400 font-mono"
        />
        {label && !labelValid && (
          <p className="text-xs text-red-400 mt-1">
...
export default function CreateServerWizard({ onClose }) {
  ...
  const startProvision = async () => {
    ...
    try {
      const entry = await invoke('provision_server', {
        ...
        label: label || `lamaste-${selectedRegion}`,
        size: selectedSize,
        domain: domain || null,
        email: email || null,
  ...
  const canNext = () => {
    switch (currentStepId) {
      ...
      case 'label': {
        const l = label || `lamaste-${selectedRegion}`;
