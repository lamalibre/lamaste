...
/// Tunnel information returned by the panel API.
...
// --- Agentd REST API helpers ---

/// Base URL for the agentd daemon running on localhost.
const AGENTD_BASE_URL: &str = "http://127.0.0.1:9393/api";

/// Build a reqwest client for agentd calls (plain HTTP, no TLS).
fn agentd_client() -> Result<reqwest::Client, String> {
...
/// Make a GET request to the agentd daemon and return the response body as JSON.
async fn agentd_get(path: &str) -> Result<serde_json::Value, String> {
    let url = format!("{}{}", AGENTD_BASE_URL, path);
    let client = agentd_client()?;
    let response = client
        .get(&url)
        .send()
        .await
        .map_err(|e| format!("Failed to reach agentd at {}: {}", url, e))?;
    ...
    if !status.is_success() {
        ...
        return Err(format!("agentd returned HTTP {}", status));
    }

    Ok(body)
}

/// Make a POST request to the agentd daemon with an optional JSON body.
async fn agentd_post(path: &str, body: Option<serde_json::Value>) -> Result<serde_json::Value, String> {
    let url = format!("{}{}", AGENTD_BASE_URL, path);
    let client = agentd_client()?;
...
/// Make a PATCH request to the agentd daemon with a JSON body.
#[allow(dead_code)]
async fn agentd_patch(path: &str, body: serde_json::Value) -> Result<serde_json::Value, String> {
    let url = format!("{}{}", AGENTD_BASE_URL, path);
    let client = agentd_client()?;
    let response = client
        .patch(&url)
...
/// Make a DELETE request to the agentd daemon.
#[allow(dead_code)]
async fn agentd_delete(path: &str) -> Result<serde_json::Value, String> {
    let url = format!("{}{}", AGENTD_BASE_URL, path);
    let client = agentd_client()?;
    let response = client
        .delete(&url)
...
pub async fn get_agents() -> Result<Vec<AgentWithStatus>, String> {
    let body = agentd_get("/agents").await?;

    let agents = body
        .get("agents")
        .and_then(|a| a.as_array())
        .ok_or("Invalid response: missing agents array")?;

    let mut result = Vec::with_capacity(agents.len());
    for agent in agents {
        result.push(AgentWithStatus {
            label: agent["label"].as_str().unwrap_or_default().to_string(),
            panel_url: agent["panelUrl"].as_str().unwrap_or_default().to_string(),
            auth_method: agent["authMethod"].as_str().unwrap_or("p12").to_string(),
            domain: agent["domain"].as_str().map(String::from),
            chisel_version: agent["chiselVersion"].as_str().map(String::from),
            setup_at: agent["setupAt"].as_str().map(String::from),
            updated_at: agent["updatedAt"].as_str().map(String::from),
            running: agent["running"].as_bool().unwrap_or(false),
            pid: agent["pid"].as_u64().map(|p| p as u32),
        });
    }
    Ok(result)
}
...
pub async fn get_agent_status(label: String) -> Result<AgentWithStatus, String> {
    ...
    let path = format!("/agents/{}", label);
    let agent = agentd_get(&path).await?;
...
pub async fn start_agent(label: String) -> Result<String, String> {
    validate_agent_label(&label)?;
    let path = format!("/agents/{}/start", label);
    agentd_post(&path, None).await?;
...
pub async fn stop_agent(label: String) -> Result<String, String> {
    validate_agent_label(&label)?;
    let path = format!("/agents/{}/stop", label);
    agentd_post(&path, None).await?;
...
pub async fn uninstall_agent(label: String) -> Result<serde_json::Value, String> {
    tokio::task::spawn_blocking(move || {
        ...
        let panel_result = Command::new("lamaste-agent")
        ...
        match panel_result {
            ...
            Err(_) => {
                add("Stop panel service", "skipped", "lamaste-agent CLI not available");
            }
        }
        ...
        // 4. Remove per-agent data directory (~/.lamalibre/lamaste/agents/<label>/)
        let data_dir = agent_data_dir(&label);
        if data_dir.exists() {
            // Symlink attack protection: verify canonical path is under ~/.lamalibre/lamaste/agents/
...
pub async fn restart_agent(label: String) -> Result<String, String> {
    validate_agent_label(&label)?;
    let path = format!("/agents/{}/restart", label);
    agentd_post(&path, None).await?;
    Ok(format!("Agent \"{}\" restarted", label))
}

#[tauri::command]
pub async fn get_agent_tunnels(label: String) -> Result<Vec<TunnelInfo>, String> {
    tokio::task::spawn_blocking(move || {
        validate_agent_label(&label)?;
...
pub async fn get_agent_logs(label: String) -> Result<String, String> {
    validate_agent_label(&label)?;
    let path = format!("/agents/{}/logs", label);
    let body = agentd_get(&path).await?;
    Ok(body["logs"].as_str().unwrap_or_default().to_string())
}

#[tauri::command]
pub async fn get_agent_config(label: String) -> Result<serde_json::Value, String> {
    validate_agent_label(&label)?;
    let path = format!("/agents/{}/config", label);
    agentd_get(&path).await
}
...
pub async fn get_panel_expose_status(label: String) -> Result<PanelExposeStatus, String> {
    tokio::task::spawn_blocking(move || {
        ...
        // Run: lamaste-agent panel --status --label <label> --json
        let output = Command::new("lamaste-agent")
            .args(["panel", "--status", "--label", &label, "--json"])
            .output()
            .map_err(|e| format!("Failed to run lamaste-agent: {}", e))?;
...
/// NDJSON progress event from lamaste-agent setup --json.
...
/// Locate Node.js and lamaste-agent CLI, installing if needed.
...
fn find_or_install_agent_cli(app: &tauri::AppHandle) -> Result<PathBuf, String> {
    ...
    let install_output = Command::new("npm")
        .args(["install", "-g", "@lamalibre/lamaste-agent@latest", "--ignore-scripts"])
        .output()
        .map_err(|e| format!("Failed to run npm install: {}", e))?;

    if !install_output.status.success() {
        // If update fails but binary already exists, use the existing version
        if find_lamaste_agent().is_none() {
            let stderr = String::from_utf8_lossy(&install_output.stderr);
            emit_install_progress(app, "install_agent_cli", "failed");
            return Err(format!(
                "Failed to install lamaste-agent: {}",
                stderr.trim()
            ));
        }
    }

    let cli_path = match find_lamaste_agent() {
        Some(p) => p,
        None => {
            ...
            if let Some(prefix) = prefix_output {
                let bin_path =
                    PathBuf::from(prefix.trim()).join("bin").join("lamaste-agent");
                if bin_path.exists() {
                    bin_path
                } else {
                    emit_install_progress(app, "install_agent_cli", "failed");
                    return Err(
                        "lamaste-agent installed but not found in PATH. Check your npm prefix configuration.".to_string()
                    );
                }
            } else {
                emit_install_progress(app, "install_agent_cli", "failed");
                return Err(
                    "lamaste-agent installed but not found in PATH".to_string(),
                );
            }
        }
    };
...
/// Find lamaste-agent binary in PATH.
fn find_lamaste_agent() -> Option<PathBuf> {
    Command::new("which")
        .arg("lamaste-agent")
        .output()
        .ok()
...
/// Install a lamaste agent by spawning `lamaste-agent setup --json`.
...
pub async fn install_agent(
    ...
    let result: Result<AgentWithStatus, String> = tokio::task::spawn_blocking(move || {
        ...
        // Phase 2: Spawn lamaste-agent setup --json
        let mut child = Command::new(&cli_path)
            ...
            .env("LAMALIBRE_LAMASTE_ENROLLMENT_TOKEN", &token)
            .stdout(std::process::Stdio::piped())
            .stderr(std::process::Stdio::inherit())
            .spawn()
            .map_err(|e| format!("Failed to start lamaste-agent: {}", e))?;

        let stdout = child.stdout.take().ok_or("No stdout from lamaste-agent")?;
        ...
        let exit_status = child
            .wait()
            .map_err(|e| format!("Failed to wait for lamaste-agent: {}", e))?;
