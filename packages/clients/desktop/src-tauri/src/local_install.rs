...
///
/// These commands allow the desktop app to install a Lamaste server directly
/// on the local Linux machine using `pkexec` for privilege escalation and the
/// `create-lamaste` installer in `--json` (NDJSON) mode.
...
const ALLOWED_PKI_PREFIX: &str = "/etc/lamalibre/lamaste/pki/";
...
/// NDJSON progress line emitted by `create-lamaste --json`.
...
/// Resolve the path to the create-lamaste CLI entry point.
...
fn create_lamaste_cli_path() -> Result<PathBuf, String> {
    ...
    let output = std::process::Command::new("which")
        .arg("create-lamaste")
        .output()
        .map_err(|e| format!("Failed to locate create-lamaste: {}", e))?;
    ...
    Err("create-lamaste not found. Install it with: npm install -g @lamalibre/create-lamaste".to_string())
}

/// Validate that a path from NDJSON output is within the allowed PKI directory.
/// Rejects path traversal (`..`), paths outside `/etc/lamalibre/lamaste/pki/`, and
...
fn validate_pki_path(path: &str) -> Result<(), String> {
    ...
    // This prevents symlink-based escapes (e.g., /etc/lamalibre/lamaste/pki/link -> /etc/shadow).
...
/// `~/.lamalibre/lamaste/servers/<id>/` directory using `pkexec` for root file access.
...
pub async fn check_local_install_available() -> Result<LocalInstallStatus, String> {
    tokio::task::spawn_blocking(|| {
        ...
        let existing_install = std::path::Path::new("/etc/lamalibre/lamaste/panel.json").exists();
...
/// Install a Lamaste server on the local machine via pkexec + create-lamaste --json.
...
pub async fn start_local_install(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        let cli_path = create_lamaste_cli_path()?;
...
/// Import an existing local Lamaste installation into the server registry
/// without running the installer. Used when /etc/lamalibre/lamaste/panel.json exists
...
pub async fn import_existing_local_install(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        if !std::path::Path::new("/etc/lamalibre/lamaste/panel.json").exists() {
            return Err(
                "No existing Lamaste installation found at /etc/lamalibre/lamaste/panel.json".to_string(),
            );
        }
        ...
        let p12_dest = import_local_certs(
            ...
            "/etc/lamalibre/lamaste/pki/.p12-password",
        )?;
...
/// Check health of the local Lamaste server at https://127.0.0.1:9292.
#[tauri::command]
