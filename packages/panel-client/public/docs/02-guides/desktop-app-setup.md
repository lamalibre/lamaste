...
Agent mode provides local agent management. The agent-mode pages (Dashboard, Tunnels, Services, Plugins, Logs, Settings) are imported from the shared `@lamalibre/portlama-agent-panel` package. The landing page is the **Agents** list, showing all configured agents with their connection status. Click an agent to drill into its management pages:
