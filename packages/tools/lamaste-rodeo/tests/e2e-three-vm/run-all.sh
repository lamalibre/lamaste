#!/usr/bin/env bash
# ============================================================================
# Lamaste Three-VM E2E — Master Test Runner
# ============================================================================
...
#   TEST_DOMAIN        — Domain configured for testing (e.g., test.lamaste.local)
...
if [ "${#MISSING[@]}" -gt 0 ]; then
  ...
  echo ""
  echo "Example:"
  echo "  HOST_IP=10.0.0.10 AGENT_IP=10.0.0.11 VISITOR_IP=10.0.0.12 TEST_DOMAIN=test.lamaste.local \\"
...
log_header "Lamaste Three-VM End-to-End Test Suite"
...
multipass exec lamaste-host -- sudo bash -c 'systemctl stop authelia; sqlite3 /etc/authelia/db.sqlite3 "DELETE FROM authentication_logs; DELETE FROM totp_history;" 2>/dev/null; systemctl start authelia' 2>/dev/null || true
sleep 3
