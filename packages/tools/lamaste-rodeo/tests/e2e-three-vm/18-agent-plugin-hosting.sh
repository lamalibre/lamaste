...
# POST /api/certs/agent writes the p12 to /etc/lamalibre/lamaste/pki/agents/<label>/client.p12
# and returns the password as .p12Password (no inline p12 blob). Copy the file
# from disk instead of decoding a field that isn't there.
P12_PASS=$(echo "$CERT_RESPONSE" | jq -r '.p12Password' 2>/dev/null || echo "")
AGENT_CERT_DIR="/tmp/agent-plugin-test-${AGENT_LABEL}"

host_exec "mkdir -p ${AGENT_CERT_DIR} && cp /etc/lamalibre/lamaste/pki/agents/${AGENT_LABEL}/client.p12 ${AGENT_CERT_DIR}/client.p12 && openssl pkcs12 -in ${AGENT_CERT_DIR}/client.p12 -out ${AGENT_CERT_DIR}/client.crt -clcerts -nokeys -passin pass:${P12_PASS} 2>/dev/null && openssl pkcs12 -in ${AGENT_CERT_DIR}/client.p12 -out ${AGENT_CERT_DIR}/client.key -nocerts -nodes -passin pass:${P12_PASS} 2>/dev/null && chmod 600 ${AGENT_CERT_DIR}/client.key"
...
REPORT_RESPONSE=$(host_agent_api_post "${AGENT_CERT_DIR}/client.crt" "${AGENT_CERT_DIR}/client.key" "agents/plugins/report" '{"plugins":[{"name":"test-sync","version":"0.1.0","capabilities":["plugin:test-sync:connect","plugin:test-sync:read"]}]}')
...
SCOPED_RESPONSE=$(host_agent_api_post "${AGENT_CERT_DIR}/client.crt" "${AGENT_CERT_DIR}/client.key" "agents/plugins/report" '{"plugins":[{"name":"myplugin","version":"1.0.0","capabilities":["plugin:myplugin:action","plugin:other:action"]}]}')
SCOPED_MERGED=$(echo "$SCOPED_RESPONSE" | jq -r '.merged' 2>/dev/null || echo "0")
assert_eq "$SCOPED_MERGED" "1" "Only plugin:myplugin:action merged (plugin:other:action rejected)" || true
...
CORE_NS_STATUS=$(host_agent_api_post_status "${AGENT_CERT_DIR}/client.crt" "${AGENT_CERT_DIR}/client.key" "agents/plugins/report" '{"plugins":[{"name":"bad","version":"1.0.0","capabilities":["tunnels:read"]}]}')
assert_eq "$CORE_NS_STATUS" "400" "Capability claiming a core namespace rejected with 400" || true
...
ADMIN_REPORT=$(host_api_post "agents/plugins/report" '{"plugins":[{"name":"admin-plugin","version":"1.0.0","capabilities":["plugin:admin-plugin:manage"]}]}')
