...
#   --domain DOMAIN  Test domain (default: test.lamaste.local)
...
VM_HOST="lamaste-host"
VM_AGENT="lamaste-agent"
VM_VISITOR="lamaste-visitor"
...
vm_ip() {
  ...
  case "$1" in
    lamaste-host)   echo "10.13.37.1"; return ;;
    lamaste-agent)  echo "10.13.37.2"; return ;;
    lamaste-visitor) echo "10.13.37.3"; return ;;
  esac
...
log_header "Lamaste Three-VM E2E Test Orchestrator"
...
if [ "$SKIP_CREATE" = "false" ]; then
  ...
  for vm_entry in "${VM_HOST}:10.13.37.1" "${VM_AGENT}:10.13.37.2" "${VM_VISITOR}:10.13.37.3"; do
    ...
    multipass exec "${vm}" -- sudo bash -c "
      printf '[Unit]\nDescription=Static IP for E2E testing\nAfter=network-online.target\nWants=network-online.target\n\n[Service]\nType=oneshot\nExecStart=/sbin/ip addr add ${ip}/24 dev enp0s1\nRemainAfterExit=yes\n\n[Install]\nWantedBy=multi-user.target\n' > /etc/systemd/system/lamaste-static-ip.service && \
      systemctl daemon-reload && systemctl enable lamaste-static-ip"
  done
...
# ============================================================================
# Phase 2: Install Lamaste and set up VMs
# ============================================================================
if [ "$SKIP_SETUP" = "false" ]; then
  ...
  # -----------------------------------------------------------------------
  log_step "Bundling vendor into create-lamaste..."
  node "${REPO_ROOT}/packages/provisioners/server/scripts/bundle-vendor.js" || \
    log_fatal "bundle-vendor.js failed"
  log_ok "Vendor bundled"

  log_step "Packing create-lamaste tarball..."
  TARBALL=$(cd "${REPO_ROOT}/packages/provisioners/server" && npm pack --pack-destination /tmp 2>/dev/null | tail -1)
  TARBALL="/tmp/${TARBALL}"
  [ -f "$TARBALL" ] || log_fatal "npm pack failed — tarball not found"
  log_ok "Tarball ready: ${TARBALL}"

  # -----------------------------------------------------------------------
  # 2a-bis: Build and pack lamaste-agent tarball
  # -----------------------------------------------------------------------
  log_step "Packing lamaste-agent tarball..."
  AGENT_TARBALL=$(cd "${REPO_ROOT}/packages/agent/cli" && npm pack --pack-destination /tmp 2>/dev/null | tail -1)
  AGENT_TARBALL="/tmp/${AGENT_TARBALL}"
  [ -f "$AGENT_TARBALL" ] || log_fatal "npm pack (lamaste-agent) failed — tarball not found"
  ...
  multipass transfer "${TARBALL}" "${VM_HOST}:/tmp/create-lamaste.tgz"
  log_ok "Tarball transferred"

  log_step "Installing create-lamaste from tarball on ${VM_HOST}..."
  run_cmd "npm install -g tarball on ${VM_HOST}" \
    multipass exec "${VM_HOST}" -- sudo npm install -g /tmp/create-lamaste.tgz || \
    log_fatal "npm install -g failed on ${VM_HOST}"

  log_step "Running create-lamaste on ${VM_HOST}..."
  set +e
  multipass exec "${VM_HOST}" -- sudo \
    create-lamaste --dev --skip-harden --yes
  INSTALLER_RC=$?
  set -e
  if [ "${INSTALLER_RC}" -ne 0 ]; then
    log_fatal "create-lamaste failed with exit code ${INSTALLER_RC}"
  fi
  log_ok "Lamaste installed on ${VM_HOST}"

  # Patch panel.json to use the static IP instead of the DHCP IP that
  # create-lamaste auto-detected via hostname -I.
  log_step "Patching panel.json with static IP (${HOST_IP})..."
  multipass exec "${VM_HOST}" -- sudo \
    sed -i "s/\"ip\": *\"[^\"]*\"/\"ip\": \"${HOST_IP}\"/" /etc/lamalibre/lamaste/panel.json
  multipass exec "${VM_HOST}" -- sudo systemctl restart lamalibre-lamaste-serverd || true
  ...
  CREDS_JSON=$(multipass exec "${VM_HOST}" -- sudo cat /tmp/lamalibre-lamaste-test-credentials.json 2>/dev/null || echo "{}")
  ...
  # -----------------------------------------------------------------------
  log_info "Transferring lamaste-agent tarball to ${VM_AGENT}..."
  multipass transfer "${AGENT_TARBALL}" "${VM_AGENT}:/tmp/lamalibre-lamaste-agent.tgz" || \
