...
cleanup_2fa() {
  ...
  # Defence in depth: prefer lamaste-reset-admin (rotates admin cert,
  # disables 2FA, re-enables IP vhost, restarts panel, reloads nginx). If
  # the binary is missing — seen when an older create-lamaste provisioned
  # the VM before the CLI was bundled — fall back to an inline 2FA-only
  # rollback. Both branches run under timed_exec so a pipe-leak cannot
  # wedge rodeo. The bash -c body uses only double quotes internally so
  # the outer single-quote delimiters stay simple.
  timed_exec 60 multipass exec lamaste-host -- sudo bash -c "
    if command -v lamaste-reset-admin >/dev/null 2>&1; then
      lamaste-reset-admin
      exit 0
    fi
    echo 'lamaste-reset-admin not found — running inline 2FA rollback' >&2
    jq '.panel2fa = {enabled: false, secret: null, setupComplete: false} | .sessionSecret = null' \
      /etc/lamalibre/lamaste/panel.json > /etc/lamalibre/lamaste/panel.json.tmp
    mv /etc/lamalibre/lamaste/panel.json.tmp /etc/lamalibre/lamaste/panel.json
    chown lamaste:lamaste /etc/lamalibre/lamaste/panel.json
    chmod 600 /etc/lamalibre/lamaste/panel.json
    if [ -f /etc/nginx/sites-available/lamalibre-lamaste-serverd-ip ]; then
      ln -sf /etc/nginx/sites-available/lamalibre-lamaste-serverd-ip /etc/nginx/sites-enabled/lamalibre-lamaste-serverd-ip
    fi
    systemctl restart lamalibre-lamaste-serverd 2>/dev/null || true
...
# `sudo` on the agent VM means the agent data dir is /root/.lamalibre/lamaste, not
# /home/ubuntu/.lamalibre/lamaste. The openssl extraction is wrapped in timed_exec
# because this multipass exec has been observed to CPU-spin for >10 min
# even though the inner commands complete or fail fast.
AGENT_HOME="/root/.lamalibre/lamaste/agents/e2e-agent"
AGENT_P12="${AGENT_HOME}/client.p12"
AGENT_CERT="/tmp/agent-e2e.crt"
AGENT_KEY="/tmp/agent-e2e.key"
timed_exec 30 multipass exec lamaste-agent -- sudo bash -c "
  P12_PASS=\$(jq -r '.p12Password' ${AGENT_HOME}/config.json 2>/dev/null)
...
# Clean up temp PEM files — wrap in timed_exec: multipass has been observed
# to pipe-leak and CPU-spin even on trivial `rm` after a prior curl exchange
# holds the agent VM's stdout handle open. See memory rule 3.
timed_exec 15 multipass exec lamaste-agent -- rm -f "${AGENT_CERT}" "${AGENT_KEY}"

# ---------------------------------------------------------------------------
log_section "Admin request without cookie returns 401"
# ---------------------------------------------------------------------------

ADMIN_NO_SESSION=$(multipass exec lamaste-host -- sudo bash -c "curl -sk -o /dev/null -w '%{http_code}' --max-time 30 --cert /etc/lamalibre/lamaste/pki/client.crt --key /etc/lamalibre/lamaste/pki/client.key --cacert /etc/lamalibre/lamaste/pki/ca.crt 'https://panel.${DOMAIN}/api/system/stats'" 2>/dev/null || echo "000")
...
if [ -n "$HOST_COOKIE" ] && [ "$HOST_COOKIE" != "null" ]; then
  ...
  AUTH_STATUS=$(multipass exec lamaste-host -- sudo bash -c "curl -sk -o /dev/null -w '%{http_code}' --max-time 30 --cert /etc/lamalibre/lamaste/pki/client.crt --key /etc/lamalibre/lamaste/pki/client.key --cacert /etc/lamalibre/lamaste/pki/ca.crt -b 'lamaste_2fa_session=${HOST_COOKIE}' 'https://panel.${DOMAIN}/api/system/stats'" 2>/dev/null || echo "000")
...
DISABLE_RESULT=$(multipass exec lamaste-host -- sudo bash -c "curl -sk --max-time 30 --cert /etc/lamalibre/lamaste/pki/client.crt --key /etc/lamalibre/lamaste/pki/client.key --cacert /etc/lamalibre/lamaste/pki/ca.crt -X POST -H 'Content-Type: application/json' -b 'lamaste_2fa_session=${HOST_COOKIE}' -d '{\"code\":\"${DISABLE_CODE}\"}' 'https://panel.${DOMAIN}/api/settings/2fa/disable'" 2>/dev/null || echo '{}')
...
IP_STATUS=$(multipass exec lamaste-host -- sudo bash -c "curl -sk -o /dev/null -w '%{http_code}' --max-time 10 --cert /etc/lamalibre/lamaste/pki/client.crt --key /etc/lamalibre/lamaste/pki/client.key --cacert /etc/lamalibre/lamaste/pki/ca.crt 'https://127.0.0.1:9292/api/health'" 2>/dev/null || echo "000")
