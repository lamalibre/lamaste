...
/**
 * CLI entry point for lamaste-cloud.
 *
 ...
 *   LAMALIBRE_CLOUD_TOKEN          — compute commands (DO API token)
 *   LAMALIBRE_SPACES_ACCESS_KEY    — storage commands (Spaces access key)
 *   LAMALIBRE_SPACES_SECRET_KEY    — storage commands (Spaces secret key)
 *
...
function usage() {
  console.error(`Usage: lamaste-cloud <command> [options]
...
Compute commands:
  ...
  discover   Discover existing Lamaste-managed droplets (resolves DNS domains)
...
Environment:
  LAMALIBRE_CLOUD_TOKEN          Cloud provider API token (required for compute commands)
  LAMALIBRE_SPACES_ACCESS_KEY    Spaces access key (required for storage commands)
  LAMALIBRE_SPACES_SECRET_KEY    Spaces secret key (required for storage commands)
`);
  process.exit(1);
}

function getToken() {
  const token = process.env.LAMALIBRE_CLOUD_TOKEN;
  if (!token) {
    console.error('Error: LAMALIBRE_CLOUD_TOKEN environment variable is required');
    process.exit(1);
  }
  return token;
}
...
async function main() {
  ...
  switch (command) {
    ...
    case 'provision-storage': {
      ...
      const accessKey = process.env.LAMALIBRE_SPACES_ACCESS_KEY;
