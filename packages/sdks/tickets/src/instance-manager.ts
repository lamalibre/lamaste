 ...
 *
 * When a plugin's server runs as a Lamaste plugin, the TicketInstanceManager
...
export class TicketInstanceManager {
  ...
  async start(): Promise<void> {
    ...
    // Start instance heartbeat. Track the in-flight promise so stop() can
    // await it and avoid mutating state after termination.
    this.heartbeatTimer = setInterval(() => {
      if (this.heartbeatInFlight) return;
      const p = this.sendHeartbeat().finally(() => {
        if (this.heartbeatInFlight === p) this.heartbeatInFlight = null;
      });
      this.heartbeatInFlight = p;
    }, this.instanceHeartbeatIntervalMs);
  ...
  async stop(): Promise<void> {
    ...
    // Drain any in-flight heartbeat so it cannot mutate state (e.g.
    // re-register on 404) after stop() returns.
    if (this.heartbeatInFlight) {
      try {
        await this.heartbeatInFlight;
      } catch {
        // sendHeartbeat already logs failures; swallow here so stop() completes.
      }
  ...
  /**
   * Close the undici dispatcher, swallowing errors. stop() must always
   * succeed, so dispatcher.close() failures are logged but never re-thrown.
   */
  private async closeDispatcher(): Promise<void> {
    ...
    try {
      ...
      this.logger.warn(
        { err: err instanceof Error ? err.message : String(err) },
        'Failed to close ticket dispatcher',
      );
    }
  ...
  private async sendHeartbeat(): Promise<void> {
    if (!this.running || !this.client || !this.instanceId) return;
    ...
    try {
      ...
      if (statusCode === 404) {
        ...
        try {
          const result = await this.client.registerInstance(this.scope, this.transport);
          // stop() may have run while registerInstance was in flight — do not
          // mutate instanceId after termination.
          if (!this.running) return;
          this.instanceId = result.instanceId;
