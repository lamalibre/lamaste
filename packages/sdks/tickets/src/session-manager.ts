 ...
 *
 * In Lamaste plugin mode, the target agent must obtain a valid ticket
...
export class TicketSessionManager {
  ...
  private pendingTransitionTimer: ReturnType<typeof setTimeout> | null = null;
  ...
  private async processTicket(ticket: TicketInboxEntry): Promise<void> {
    try {
      ...
      if (!validation.valid) {
        // Reached only if the panel returns HTTP 200 with `{ valid: false }`.
        // The current panel returns 401 on denial (handled in the catch
        // branch below). Logged with the same structured context either way
        // so operators can diagnose stuck inboxes.
        this.logger.warn(
          {
            ticketId: validation.ticketId ?? ticket.id.slice(0, 8) + '...',
            reason: validation.reason ?? 'unknown',
            scope: ticket.scope,
            source: ticket.source,
          },
          'Ticket validation returned invalid, resuming inbox poll',
        );
        this.scheduleInboxPoll(this.inboxPollIntervalMs);
        return;
      }
      ...
      // Same structured context as the !valid branch so journalctl filters
      // pick up either denial path (401 today, body-encoded valid:false in
      // a future panel revision).
