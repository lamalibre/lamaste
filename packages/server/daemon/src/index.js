...
async function start() {
  ...
  // Wire ticket module to the Fastify root logger so any load-time warnings
  ...
  if (config.staticDir) {
    ...
    staticRoot = path.resolve(__dirname, '..', '..', 'ui', 'dist');
  } else {
    staticRoot = path.join(config.dataDir, 'server-ui', 'dist');
  }
  ...
  // Serve docs from a separate directory (version-stamped on server, raw on dev)
  let docsRoot;
  if (isDev) {
    docsRoot = path.resolve(__dirname, '..', '..', '..', 'core', 'docs');
  } else if (config.staticDir) {
    // staticDir is <installDir>/server-ui/dist — docs live at <installDir>/docs
    docsRoot = path.join(config.staticDir, '..', '..', 'docs');
  } else {
    docsRoot = path.join(config.dataDir, 'docs');
  }
  await server.register(fastifyStatic, {
    root: docsRoot,
    prefix: '/docs/',
