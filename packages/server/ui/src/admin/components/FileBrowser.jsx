...
export default function FileBrowser({ site, onBack }) {
  ...
  const filesQuery = useQuery({
    ...
    queryFn: () => client.getSiteFiles(site.id, currentPath),
    refetchInterval: 5_000,
  });

  const uploadMutation = useMutation({
    mutationFn: ({ files }) => client.uploadSiteFiles(site.id, currentPath, files),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ['site-files', site.id] });
      queryClient.invalidateQueries({ queryKey: ['sites'] });
      const count = data?.files?.length ?? 0;
      const msg = data?.warning
        ? `${count} file(s) uploaded. ${data.warning}`
  ...
  const deleteMutation = useMutation({
    mutationFn: (filePath) => client.deleteSiteFile(site.id, filePath),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['site-files', site.id] });
      queryClient.invalidateQueries({ queryKey: ['sites'] });
