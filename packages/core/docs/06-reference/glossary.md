...
> A-Z definitions of every term used in Lamaste documentation, each explained at three levels of depth.

## In Plain English

This page is a dictionary for Lamaste. Every technical term you encounter in the documentation is defined here. Each entry has three parts:

- **Plain English**: A non-technical explanation anyone can understand
- **Technical**: The precise definition for developers
- **Where it appears**: Where you encounter this term in Lamaste
...
**Where it appears:** Lamaste uses atomic writes for `panel.json`, `tunnels.json`, and Authelia's `users.yml`. This is critical because Authelia reads `users.yml` on every authentication request — a half-written file would break login.
...
**Plain English:** A trusted stamp-maker. The CA creates a master stamp, and any certificate it stamps is considered trustworthy. In Lamaste, the CA is self-created and exists only on your droplet.

**Technical:** A Certificate Authority issues and signs digital certificates. Lamaste generates a self-signed CA during installation (4096-bit RSA key, 10-year validity) stored at `/etc/lamalibre/lamaste/pki/ca.key` and `/etc/lamalibre/lamaste/pki/ca.crt`. This CA signs client certificates for mTLS admin authentication. nginx is configured to trust only certificates signed by this CA. The CA is distinct from Let's Encrypt, which is used for public-facing TLS.

**Where it appears:**

- Generated by the installer's mTLS certificate task
- Referenced in `/etc/nginx/snippets/lamalibre-lamaste-mtls.conf` as `ssl_client_certificate`
...
**Technical:** The official ACME client for Let's Encrypt. Lamaste uses certbot with the nginx plugin (`python3-certbot-nginx`) to obtain and auto-renew TLS certificates for the configured domain and its wildcard subdomains. Certificates are stored in `/etc/letsencrypt/`. A systemd timer (`certbot.timer`) runs renewal checks twice daily.
...
**Technical:** Chisel is an open-source tunneling tool written in Go that creates TCP tunnels over WebSocket connections. In Lamaste, the Chisel server runs on the droplet (binding `127.0.0.1:9090`) and the Chisel client runs on your machine (macOS or Linux). The tunnel is established as a WebSocket upgrade inside a standard HTTPS connection, making it indistinguishable from normal web traffic to deep packet inspection (DPI) systems. The server uses approximately 20MB of RAM.
...
**Technical:** An X.509 certificate installed in the browser's certificate store. During TLS handshake, the server requests the client's certificate (`ssl_verify_client on`), and the browser presents it. If the certificate is signed by the trusted CA, the connection proceeds. If not, the TLS handshake fails before any HTTP traffic occurs. Lamaste generates a PKCS12 (`.p12`) bundle containing the client certificate, private key, and CA certificate for easy browser import.
...
**Technical:** A hierarchical naming system that translates human-readable domain names to IP addresses. Lamaste requires two DNS records: an `A` record for the base domain and a wildcard `A` record (`*.example.com`) pointing to the droplet's IP. The wildcard allows creating tunnels like `app.example.com` without adding individual DNS records for each subdomain.
...
**Plain English:** DigitalOcean's name for a virtual server in the cloud. Lamaste uses the cheapest one available as a relay point.

**Technical:** A virtual machine instance on DigitalOcean's infrastructure. Lamaste requires the $4/month tier (512MB RAM, 1 vCPU, 10GB SSD) running Ubuntu 24.04 LTS. While DigitalOcean is the reference platform, Lamaste works on any VPS provider with a public IP and Ubuntu 24.04.

**Where it appears:**

- Created by the user as the first step of setup
- Hosts all Lamaste server-side components
...
**Technical:** A Node.js library for executing external processes. It replaces `child_process.exec/spawn` with a promise-based API, proper error handling, and array-based argument passing (preventing shell injection). Lamaste uses execa exclusively for all system command execution — `child_process` is never used directly.

**Where it appears:**

- Used throughout `create-lamaste` installer tasks
- Used in `lamalibre-lamaste-serverd` business logic (`lib/`) for nginx, certbot, systemctl operations
...
**Technical:** An intrusion prevention framework that monitors log files for patterns indicating brute-force attacks and temporarily bans offending IP addresses using firewall rules. Lamaste configures two jails: `sshd` (monitors `/var/log/auth.log`) and `nginx-http-auth` (monitors `/var/log/nginx/error.log`). Both jails ban IPs for 3600 seconds (1 hour) after 5 failed attempts. Uses approximately 35MB of RAM.

**Where it appears:**

- Installed and configured during the installer's hardening step
- Configuration: `/etc/fail2ban/jail.d/lamaste.conf`
...
**Plain English:** The engine that powers the Lamaste admin panel's backend. It receives requests from your browser and responds with data, like a restaurant kitchen receiving orders and sending out dishes.

**Technical:** A high-performance Node.js web framework. Lamaste uses Fastify for the panel server REST API because it provides built-in schema validation (used with Zod), native WebSocket support (for live log streaming), and a structured plugin system. The panel server binds to `127.0.0.1:3100` and is accessed only through the nginx reverse proxy. Routes handle HTTP concerns only; business logic lives in `lib/` modules.

**Where it appears:**

- Panel server entry point: `packages/lamaste-serverd/src/index.js`
- Routes: `packages/lamaste-serverd/src/routes/`
- Business logic: `packages/lamaste-serverd/src/lib/`
- Runs as the `lamalibre-lamaste-serverd` systemd service
...
**Technical:** A domain name that specifies its exact location in the DNS hierarchy, including all labels up to the root. For example, `myapp.example.com.` is an FQDN (the trailing dot is the root, often omitted). In Lamaste, each tunnel gets an FQDN like `myapp.example.com` that resolves to the droplet's IP through the wildcard DNS record.
...
**Plain English:** The macOS system that automatically starts programs when your Mac boots up and restarts them if they crash. Lamaste uses it to keep the tunnel client running permanently.

**Technical:** The macOS init system and service manager (equivalent to systemd on Linux). Lamaste provides a launchd property list (`.plist`) file for the Chisel client that configures it to start at boot, auto-restart on failure, and run in the background. The plist is downloaded from the panel's Tunnels page after creating a tunnel.
...
**Technical:** A Node.js library for building terminal task lists with nested subtasks, progress indicators, and concurrent execution support. The Lamaste installer uses Listr2 to orchestrate all installation tasks in a visually structured pipeline. Each task can have skip guards (for idempotency) and persistent output messages.

**Where it appears:**

- Used exclusively in `create-lamaste` (`packages/create-lamaste/src/index.js`)
- Each installer phase is a Listr2 task with nested subtasks
- Task files in `packages/create-lamaste/src/tasks/` return Listr2 subtask arrays
...
**Technical:** Mutual Transport Layer Security extends standard TLS by requiring both parties to present certificates during the handshake. In Lamaste, nginx on port 9292 is configured with `ssl_verify_client on`, meaning the TLS handshake fails before any HTTP traffic if the client does not present a valid certificate signed by the Lamaste CA. This provides authentication at the transport layer — there is no login form to brute-force, no session cookies to steal, and no credentials transmitted over the wire.

**Where it appears:**

- Configured in `/etc/nginx/snippets/lamalibre-lamaste-mtls.conf`
- Enforced on the admin panel vhost (port 9292)
- Client certificates generated during installation and importable as `.p12` files
- LXD (the Linux container manager) uses the same pattern, which inspired Lamaste's approach
...
- Panel client: `packages/lamaste-server-ui/src/pages/onboarding/`
- Panel server routes: `packages/lamaste-serverd/src/routes/onboarding/`
...
**Technical:** A binary archive format (also known as PFX) defined by RFC 7292 for storing a private key, certificate, and optionally the CA certificate chain, all encrypted with a password. Lamaste generates the `.p12` bundle using OpenSSL with legacy-compatible encryption (`PBE-SHA1-3DES`) and MAC algorithm (`sha1`) to ensure compatibility with macOS Keychain, which does not support newer encryption modes for PKCS12 import.

**Where it appears:**

- Generated during installation at `/etc/lamalibre/lamaste/pki/client.p12`
- Password stored at `/etc/lamalibre/lamaste/pki/.p12-password` (mode 0600)
...
**Technical:** The panel consists of two packages. The **panel server** is a Fastify REST API running as the `lamalibre-lamaste-serverd` systemd service, binding `127.0.0.1:3100`. It handles all business logic: nginx vhost management, Chisel tunnel configuration, Authelia user management, certbot operations, and system monitoring. The **panel client** is a React 18 SPA built with Vite and Tailwind CSS, served as static files from `/opt/lamalibre/lamaste/lamaste-server-ui/dist/`. It communicates with the panel server via `/api/*` routes proxied through nginx.

**Where it appears:**

- Server: `packages/lamaste-serverd/`, deployed to `/opt/lamalibre/lamaste/lamaste-serverd/`
- Client: `packages/lamaste-server-ui/`, built to `/opt/lamalibre/lamaste/lamaste-server-ui/dist/`
...
**Technical:** An algorithm defined in RFC 6238 that generates short-lived codes from a shared secret and the current time. The server and the client (authenticator app) independently compute the same code because they share the same secret and clock. Lamaste uses TOTP through Authelia for all tunneled app authentication. The shared secret is provisioned via QR code during user creation.
...
- Definitions stored in `/etc/lamalibre/lamaste/tunnels.json`
...
**Plain English:** A lock on the droplet's doors. UFW closes all doors except the three Lamaste needs: SSH (port 22), web traffic (port 443), and the admin panel (port 9292). Everything else is blocked.

**Technical:** A frontend for Linux's iptables/nftables firewall. Lamaste configures UFW to deny all incoming traffic by default and allow only three ports: 22/tcp (SSH), 443/tcp (HTTPS for tunneled apps), and 9292/tcp (admin panel). Outgoing traffic is unrestricted (needed for package updates, Let's Encrypt validation, etc.).
...
**Technical:** An nginx `server` block that matches requests by `server_name` (the `Host` header). Lamaste creates one vhost per tunnel at `/etc/nginx/sites-available/lamalibre-lamaste-app-<subdomain>` and symlinks it to `/etc/nginx/sites-enabled/`. The admin panel also has its own vhost at `/etc/nginx/sites-available/lamalibre-lamaste-panel-ip`. After any vhost change, the panel server runs `nginx -t` to validate the configuration before reloading — if validation fails, the change is rolled back.

**Where it appears:**

- Created automatically when tunnels are added through the panel
- Files: `/etc/nginx/sites-available/lamaste-*`
...
**Technical:** A TypeScript-first schema validation library. Lamaste uses Zod at the Fastify route level to validate all incoming API request bodies, query parameters, and URL parameters. Schemas are defined inline in route files and used with Fastify's schema-first validation hooks. This ensures that business logic in `lib/` modules receives well-typed, validated data.

**Where it appears:**

- Used in `packages/lamaste-serverd/src/routes/` for request validation
...
- [What is Lamaste?](../00-introduction/what-is-lamaste.md) — project overview
