...
4. **Saves the tunnel** — Writes the tunnel record to `/etc/lamalibre/lamaste/tunnels.json`.
...
Click the **Download Mac Plist** button at the top of the tunnels page. This downloads a `com.lamalibre.lamaste.chisel.plist` file containing all current tunnel port mappings.
...
mv ~/Downloads/com.lamalibre.lamaste.chisel.plist ~/Library/LaunchAgents/
...
The Mac plist is generated by `packages/lamaste-serverd/src/lib/plist.js`. It creates a launchd configuration that:
