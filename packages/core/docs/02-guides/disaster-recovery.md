...
1. **Access the panel via IP.** This is the key design principle of Lamaste: `https://<ip>:9292` always works, regardless of DNS state (unless the optional panel 2FA is enabled, which disables the IP vhost — see Scenario 6 for recovery). Open:
...
**Critical memory rule:** Authelia must use **bcrypt** for password hashing, not argon2id. Argon2id uses ~93 MB per hash operation, which causes immediate OOM kills on a 512 MB droplet. The Lamaste installer configures bcrypt by default.
...
```bash
openssl x509 -in /etc/lamalibre/lamaste/pki/client.crt -noout -enddate
```

3. **Generate a new client certificate:**

```bash
cd /etc/lamalibre/lamaste/pki
...
# Create CSR
openssl req -new -key client.key -out client.csr -subj '/CN=Lamaste Client/O=Lamaste'
...
2. **Check panel logs** for errors mentioning `ticket-scopes.json` or `tickets.json`:
   - From the Services page, view the `lamalibre-lamaste-serverd` logs
   - Or via SSH: `journalctl -u lamalibre-lamaste-serverd -n 50`
...
```bash
echo '{"scopes":[],"instances":[],"assignments":[]}' | sudo tee /etc/lamalibre/lamaste/ticket-scopes.json
sudo chown lamaste:lamaste /etc/lamalibre/lamaste/ticket-scopes.json
sudo chmod 600 /etc/lamalibre/lamaste/ticket-scopes.json
```

Reset the ticket store (removes all tickets and sessions):

```bash
echo '{"tickets":[],"sessions":[]}' | sudo tee /etc/lamalibre/lamaste/tickets.json
sudo chown lamaste:lamaste /etc/lamalibre/lamaste/tickets.json
sudo chmod 600 /etc/lamalibre/lamaste/tickets.json
```
...
```bash
systemctl status lamalibre-lamaste-serverd chisel authelia nginx
```

View recent panel logs:

```bash
journalctl -u lamalibre-lamaste-serverd --no-pager -n 50
```

Restart all Lamaste services:

```bash
systemctl restart nginx chisel authelia lamalibre-lamaste-serverd
```
...
```bash
cat /etc/lamalibre/lamaste/panel.json | python3 -m json.tool
```
...
Lamaste is architected for recovery:

- **IP:9292 always works (unless panel 2FA is enabled).** The IP-based panel vhost uses a self-signed certificate with 10-year validity and is independent of DNS or Let's Encrypt. Even if every domain-based service fails, the admin panel is accessible. When the optional panel 2FA is enabled, the IP vhost is disabled (domain-only access). Running `sudo lamaste-reset-admin` clears 2FA and re-enables the IP vhost.
- **Systemd restarts.** All services (`lamalibre-lamaste-serverd`, `chisel`, `authelia`) have `Restart=always` and `RestartSec=5` in their systemd units. A single crash is invisible to the user.
