...
> Install and configure the Chisel tunnel client on your Mac so local apps are accessible through Lamaste.

> **Prefer the Desktop App?** The [Lamaste Desktop App](desktop-app-setup.md) provides a native GUI with automatic service discovery, one-click tunnel creation, and no manual configuration. Install it with `npx @lamalibre/create-lamaste-desktop`. This guide covers the manual CLI-based setup as an alternative.

## In Plain English

Your Lamaste server is a relay — but it needs something on your Mac to connect to it and forward traffic. That something is the Chisel client, a small program that establishes a secure WebSocket connection from your Mac to your server. Once connected, any web app running on your Mac becomes accessible through your Lamaste domain.
...
- A completed [Lamaste onboarding](onboarding.md) with at least one [tunnel created](first-tunnel.md)
- A **Mac** running macOS 12 (Monterey) or later
- **Homebrew** installed ([brew.sh](https://brew.sh))
- An **agent certificate** (`.p12` file and password) — ask your Lamaste admin to generate one from the panel (Certificates page, Agent Certificates section). See [Certificate Management](certificate-management.md#agent-certificates) for details.
...
```bash
lamaste-agent setup --token <token> --panel-url https://<ip>:9292
```
...
The Lamaste panel generates a launchd plist file configured with all your current tunnel port mappings.

**Option A — Download from the panel UI:**

1. Log in to the Lamaste panel.
2. Go to the **Tunnels** page.
3. Click the **Download Mac Plist** button.
4. Save the file `com.lamalibre.lamaste.chisel.plist`.
...
curl -k --cert-type P12 --cert macbook-pro.p12:<password> \
  ...
  -o com.lamalibre.lamaste.chisel.plist
```
...
```bash
cat com.lamalibre.lamaste.chisel.plist
```
...
<dict>
    <key>Label</key>
    <string>com.lamalibre.lamaste.chisel</string>
...
- **Label:** `com.lamalibre.lamaste.chisel` — the launchd service identifier
...
```bash
cp com.lamalibre.lamaste.chisel.plist ~/Library/LaunchAgents/
```

Load the service:

```bash
launchctl load ~/Library/LaunchAgents/com.lamalibre.lamaste.chisel.plist
```

The Chisel client starts immediately and connects to your Lamaste server.
...
```
-	0	com.lamalibre.lamaste.chisel
```
...
The plist is generated server-side by `packages/lamaste-serverd/src/lib/plist.js`. The `generatePlist()` function takes the current tunnel list and domain, and produces a complete XML plist.
