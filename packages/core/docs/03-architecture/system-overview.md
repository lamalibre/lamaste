...
│  │                                                                │  │
│  │  nginx (:443 + :9292)                                          │  │
│  │   ├─ <ip>:9292           → mTLS → lamalibre-lamaste-serverd (always on)     │  │
│  │   ├─ panel.example.com   → mTLS → lamalibre-lamaste-serverd (after setup)   │  │
...
│  │   ├─ Fastify REST API                                          │  │
│  │   ├─ Serves React UI (static files from lamaste-server-ui build)    │  │
...
│  │                                                                │  │
│  │   Lamaste Desktop (Tauri v2) — dual mode                      │  │
│  │    ├─ Agent mode: service discovery + tunnel management        │  │
│  │    │   (pages from @lamalibre/lamaste-agent-ui)            │  │
│  │    ├─ Server mode: full admin panel (via lamaste-server-ui) │  │
│  │    ├─ Multi-server registry (~/.lamalibre/lamaste/servers.json)         │  │
│  │    ├─ Cloud provisioning (DigitalOcean via lamaste-cloud)     │  │
...
/etc/nginx/
├── sites-available/
│   ├── lamalibre-lamaste-panel-ip   ← IP:9292 mTLS vhost (always present)
│   ├── lamalibre-lamaste-panel-domain ← panel.domain.com vhost (after onboarding)
│   ├── lamalibre-lamaste-auth   ← auth.domain.com vhost (after onboarding)
│   ├── lamalibre-lamaste-tunnel ← tunnel.domain.com vhost (after onboarding)
│   ├── lamalibre-lamaste-app-*  ← per-tunnel app vhosts (dynamic)
│   └── lamalibre-lamaste-site-* ← per-static-site vhosts (dynamic)
├── sites-enabled/             ← symlinks to above
└── snippets/
    └── lamalibre-lamaste-mtls.conf  ← shared mTLS directives
...
├── lamaste-serverd/           ← deployed panel backend
└── lamaste-server-ui/           ← built React static files
```
