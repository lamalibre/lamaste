...
sequenceDiagram
    ...
    Mac->>Mac: npm pack create-lamaste
    Mac->>H: transfer tarball
    Mac->>H: npm install -g + create-lamaste --dev
...
sequenceDiagram
    ...
    Mac->>H: systemctl is-active nginx, chisel, authelia, lamalibre-lamaste-serverd
    H-->>Mac: active (all 4 services)

    Mac->>H: ls /etc/lamalibre/lamaste/pki/{ca.crt, client.crt, client.key}
    H-->>Mac: files exist
...
sequenceDiagram
    ...
    H-->>Mac: [nginx, chisel, authelia, lamalibre-lamaste-serverd]
    Mac->>H: GET /api/certs
...
sequenceDiagram
    ...
    H-->>Mac: { id, fqdn }
    Mac->>H: write marker HTML to /var/www/lamaste/{id}/index.html
    Mac->>H: POST /api/users/testuser/reset-totp (mTLS)
    H-->>Mac: TOTP secret
