...
> The lamaste-server-ui is a React 18 SPA with a dark terminal aesthetic that provides the complete management interface for Lamaste, from onboarding through daily operations.

## In Plain English

The panel client is what you see in your browser when you open the Lamaste admin panel. It is a single-page application — one HTML file that loads JavaScript which handles all navigation and UI rendering in the browser. It communicates with the panel server through REST API calls and WebSocket connections.
...
The management-mode admin pages (Dashboard, Tunnels, Sites, Users, Certificates, Services, Plugins, Tickets, Logs, Settings) live in `@lamalibre/lamaste-server-ui`. This package provides both the browser SPA build and the shared admin components consumed by `lamaste-desktop` (Tauri app).

The agent-mode pages (Dashboard, Tunnels, Services, Plugins, Logs, Settings) are extracted into a separate shared package: `@lamalibre/lamaste-agent-ui`. This package is consumed by `lamaste-desktop` for agent mode.
...
- **lamaste-server-ui (web):** Uses `apiFetch` — standard browser `fetch` calls to `/api/*` endpoints, relying on the browser's mTLS client certificate for authentication.
- **lamaste-desktop (Tauri):** Uses Tauri `invoke` commands — the Rust backend handles mTLS, makes the API calls, and returns the data to the React frontend via IPC.
...
Browser
  │
  └── index.html (loaded from lamalibre-lamaste-serverd via @fastify/static)
        │
        └── React Application
              │
...
2. **AdminClientProvider** — from `@lamalibre/lamaste-server-ui`, supplies the host-specific data client (`webAdminClient` for browser fetch, Tauri `invoke` for desktop)
3. **ToastProvider** — from `@lamalibre/lamaste-server-ui`, notification toast system
4. **TwoFaProvider** — from `@lamalibre/lamaste-server-ui`, handles 2FA verification prompts when the panel has 2FA enabled
...
The documentation system is independent of the lamalibre-lamaste-serverd API — docs load even if the server is unresponsive.
...
The built `dist/` directory is served by the lamalibre-lamaste-serverd via `@fastify/static`. In production, no Vite dev server runs.
...
Markdown files in `public/docs/` are copied as-is to the build output. They can be read on GitHub, edited without build steps, and loaded independently of the lamalibre-lamaste-serverd API. The documentation viewer is a simple `fetch + marked` rendering — no SSG, no MDX compilation, no build-time processing.
