...
> List, renew, rotate, and download the TLS and mTLS certificates that secure Lamaste.

## In Plain English

Lamaste manages two kinds of certificates:
...
2. **mTLS client certificates** — this is the special certificate in your browser that proves you are the admin. It is self-signed by Lamaste's own certificate authority and used to access the management panel.
...
Checks the status of the certbot auto-renewal systemd timer. Lamaste looks for both `certbot.timer` and `certbot-renew.timer` (different distributions use different names).
...
- **Path:** `/etc/lamalibre/lamaste/pki/agents/<label>/client.p12`
...
- **Path:** `/etc/lamalibre/lamaste/pki/agents/plugin-agent:<delegatingLabel>:<pluginAgentLabel>/client.p12` (generated during CSR enrollment)
