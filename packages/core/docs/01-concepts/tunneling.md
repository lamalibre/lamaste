...
3. Lamaste issues a TLS certificate for `myapp.example.com` via Let's Encrypt
4. Lamaste writes an nginx vhost configuration to route `myapp.example.com` traffic
5. Lamaste restarts the tunnel server to pick up the new mapping
6. You run `lamaste-agent setup` on your machine (or download the Mac plist manually)
...
The Chisel client runs as a system service managed by `lamaste-agent`. On macOS it uses a launchd plist; on Linux it uses a systemd unit. The client command looks like:
...
```javascript
// From packages/lamaste-serverd/src/lib/chisel.js
...
Chisel is installed from GitHub releases during the onboarding provisioning step — not during the initial `npx @lamalibre/create-lamaste` install. The installer downloads the latest `linux_amd64` binary:
...
npx @lamalibre/lamaste-agent setup
...
systemctl status lamaste-chisel

# View recent logs
journalctl -u lamaste-chisel -n 50 --no-pager
```
