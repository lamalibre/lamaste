...
const CreateInvitationSchema = z.object({
  username: z
    ...
    .min(2)
    .max(32)
    .regex(/^[a-z0-9_-]+$/, 'Username must contain only lowercase alphanumeric characters, underscores, and hyphens'),
...
export default async function invitationRoutes(fastify, _opts) {
  ...
  fastify.post('/invitations', {
    ...
    if (existingPending) {
      return reply.code(409).send({ error: 'A pending invitation already exists for this username' });
    }
    ...
    const inviteUrl = config.domain
      ? `https://auth.${config.domain}/invite/${token}`
      : null;
