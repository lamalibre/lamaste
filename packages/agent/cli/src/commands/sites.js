...
import { requireAgentConfig } from '@lamalibre/lamaste/agent';
...
export async function runSites(args, { label } = {}) {
  ...
  if (sub && sub !== 'list') {
    ...
    console.error(`  Usage: ${chalk.cyan('lamaste-agent sites [list|create|delete]')}\n`);
    process.exit(1);
  }
...
async function runCreate(config, args) {
  ...
  if (!name) {
    console.error(
      `\n  Usage: ${chalk.cyan('lamaste-agent sites create <name> [--type managed|custom] [--domain <fqdn>] [--spa] [--auth]')}\n`,
    );
    process.exit(1);
  }
  ...
  if (type === 'custom') {
    ...
    console.log(chalk.dim(`    Create a CNAME record pointing ${fqdn} to your Lamaste server.`));
  }
...
async function runDelete(config, args) {
  ...
  if (!target) {
    console.error(`\n  Usage: ${chalk.cyan('lamaste-agent sites delete <name|uuid>')}\n`);
    process.exit(1);
  }
