...
host_exec "curl -skf --max-time 10 --cert /etc/portlama/pki/client.crt --key /etc/portlama/pki/client.key --cacert /etc/portlama/pki/ca.crt https://127.0.0.1:9292/api/onboarding/status" | jq -r '.status' | grep -q COMPLETED || {
...
if echo "$PANEL_HEALTH" | jq -r '.status' 2>/dev/null | grep -q "ok"; then
  ...
  agent_exec "portlama-agent panel --enable --port ${AGENT_PANEL_PORT} 2>/dev/null" || true
  sleep 5
  PANEL_HEALTH=$(agent_exec "curl -sf --max-time 5 http://127.0.0.1:${AGENT_PANEL_PORT}/api/health 2>/dev/null" || echo "")
  if echo "$PANEL_HEALTH" | jq -r '.status' 2>/dev/null | grep -q "ok"; then
