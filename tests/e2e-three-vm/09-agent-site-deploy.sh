...
host_agent_api_upload() {
  ...
  local api_path="$3"
  local file_path="$4"
  host_exec "curl -skf --max-time 30 --cert ${cert_path} --key ${key_path} --cacert /etc/portlama/pki/ca.crt -X POST -F 'file=@${file_path}' -H 'Accept: application/json' https://127.0.0.1:9292/api/${api_path}"
}
...
host_agent_api_upload_status() {
  ...
  local api_path="$3"
  local file_path="$4"
  host_exec "curl -sk -o /dev/null -w '%{http_code}' --max-time 30 --cert ${cert_path} --key ${key_path} --cacert /etc/portlama/pki/ca.crt -X POST -F 'file=@${file_path}' -H 'Accept: application/json' https://127.0.0.1:9292/api/${api_path}" 2>/dev/null || echo "000"
}
...
# P12 file is at /etc/portlama/pki/agents/<label>/client.p12
...
NOPERM_P12_PATH="/etc/portlama/pki/agents/${NOPERM_LABEL}/client.p12"
