...
# ============================================================================
# Verifies that create-portlama --json produces valid NDJSON output on the
# host VM. Since Portlama is already installed, this runs in redeploy mode.
# ============================================================================
...
# ---------------------------------------------------------------------------
log_section "create-portlama --json on host VM (redeploy mode)"
# ---------------------------------------------------------------------------
...
host_exec "create-portlama --json > /tmp/json-install-output.txt 2>/dev/null; true"

JSON_OUTPUT=$(host_exec "cat /tmp/json-install-output.txt 2>/dev/null" || true)

if [ -z "$JSON_OUTPUT" ]; then
  log_fail "No NDJSON output from create-portlama --json on host VM"
  end_test
  exit 1
fi
...
for i in 1 2 3 4 5; do
  sleep 3
  HEALTH=$(host_exec "curl -sk --max-time 10 --cert /etc/portlama/pki/client.crt --key /etc/portlama/pki/client.key --cacert /etc/portlama/pki/ca.crt https://127.0.0.1:9292/api/health 2>/dev/null" || echo '{}')
