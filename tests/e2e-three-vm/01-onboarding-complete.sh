...
# - All core services are running (nginx, chisel, authelia, portlama-panel)
...
SERVICES=(nginx chisel authelia portlama-panel)
...
CERT_PATHS=(
  "/etc/portlama/pki/ca.crt"
  "/etc/portlama/pki/ca.key"
  "/etc/portlama/pki/client.crt"
  "/etc/portlama/pki/client.key"
  "/etc/portlama/pki/self-signed.pem"
  "/etc/portlama/pki/self-signed-key.pem"
)
...
PANEL_STATUS=$(host_exec "curl -sk -o /dev/null -w '%{http_code}' --max-time 15 --cert /etc/portlama/pki/client.crt --key /etc/portlama/pki/client.key --cacert /etc/portlama/pki/ca.crt https://panel.${TEST_DOMAIN}" || echo "000")
