...
host_api_put() {
  host_exec "curl -skf --max-time 30 --cert /etc/portlama/pki/client.crt --key /etc/portlama/pki/client.key --cacert /etc/portlama/pki/ca.crt -X PUT -H 'Content-Type: application/json' -H 'Accept: application/json' -d '$2' https://127.0.0.1:9292/api/$1"
}
...
IP_RESPONSE=$(host_exec "curl -skf --max-time 30 --cert /etc/portlama/pki/client.crt --key /etc/portlama/pki/client.key --cacert /etc/portlama/pki/ca.crt https://127.0.0.1:9292/ 2>/dev/null" || echo "")
IP_STATUS=$(host_exec "curl -skf -o /dev/null -w '%{http_code}' --max-time 30 --cert /etc/portlama/pki/client.crt --key /etc/portlama/pki/client.key --cacert /etc/portlama/pki/ca.crt https://127.0.0.1:9292/ 2>/dev/null" || echo "000")
...
DOMAIN_RESPONSE=$(host_exec "curl -skf --max-time 30 --cert /etc/portlama/pki/client.crt --key /etc/portlama/pki/client.key --cacert /etc/portlama/pki/ca.crt https://panel.${TEST_DOMAIN}/ 2>/dev/null" || echo "")
DOMAIN_STATUS=$(host_exec "curl -skf -o /dev/null -w '%{http_code}' --max-time 30 --cert /etc/portlama/pki/client.crt --key /etc/portlama/pki/client.key --cacert /etc/portlama/pki/ca.crt https://panel.${TEST_DOMAIN}/ 2>/dev/null" || echo "000")
...
EXPECTED_SERVICES=("nginx" "chisel" "authelia" "portlama-panel")
