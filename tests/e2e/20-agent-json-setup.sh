...
# ============================================================================
# Verifies that portlama-agent setup --json produces valid NDJSON output:
...
# Find portlama-agent binary
AGENT_BIN=$(which portlama-agent 2>/dev/null || echo "")
if [ -z "$AGENT_BIN" ]; then
  # Try npm global prefix
  NPM_PREFIX=$(npm config get prefix 2>/dev/null || echo "")
  if [ -n "$NPM_PREFIX" ] && [ -f "$NPM_PREFIX/bin/portlama-agent" ]; then
    AGENT_BIN="$NPM_PREFIX/bin/portlama-agent"
  else
    log_skip "portlama-agent not found in PATH"
    end_test
    exit $?
  fi
fi

log_pass "portlama-agent found at: $AGENT_BIN"
...
NO_TOKEN_OUTPUT=$(PORTLAMA_ENROLLMENT_TOKEN="" "$AGENT_BIN" setup --json --panel-url https://127.0.0.1:9292 2>/dev/null || true)
...
# ---------------------------------------------------------------------------
log_section "portlama-agent setup --json (token-based)"
# ---------------------------------------------------------------------------

JSON_OUTPUT=$(PORTLAMA_ENROLLMENT_TOKEN="$TOKEN" \
