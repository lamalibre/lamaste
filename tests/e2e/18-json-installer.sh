...
# ============================================================================
# Verifies that create-portlama --json produces valid NDJSON output:
...
#
# Since the system already has Portlama installed, this runs in redeploy mode.
# ============================================================================
...
# ---------------------------------------------------------------------------
log_section "create-portlama --json (redeploy mode)"
# ---------------------------------------------------------------------------

# Run the installer in JSON mode and capture NDJSON output
INSTALLER_PATH="/opt/portlama/create-portlama/bin/create-portlama.js"
if [ ! -f "$INSTALLER_PATH" ]; then
  # Fallback: find it via npx or global install
  INSTALLER_PATH=$(which create-portlama 2>/dev/null || echo "")
  if [ -z "$INSTALLER_PATH" ]; then
    log_skip "create-portlama not found in PATH or /opt/portlama"
    end_test
    exit 0
  fi
fi
...
if [ -z "$JSON_OUTPUT" ]; then
  log_fail "No NDJSON output from create-portlama --json"
  end_test
  exit 1
fi
