...
for f in /etc/portlama/tunnels.json /etc/portlama/sites.json; do
  if [ -f "$f" ]; then
...
if [ -f /etc/portlama/panel.json ]; then
  PERMS=$(stat -c '%a' /etc/portlama/panel.json 2>/dev/null || stat -f '%Lp' /etc/portlama/panel.json 2>/dev/null || echo "unknown")
  if [ "$PERMS" = "640" ]; then
    log_pass "panel.json has correct permissions (640)"
  else
    log_fail "panel.json has permissions $PERMS (expected 640)"
  fi
fi
