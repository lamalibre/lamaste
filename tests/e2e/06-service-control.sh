...
# - Test: cannot stop portlama-panel from the UI
...
# ---------------------------------------------------------------------------
log_section "Cannot stop portlama-panel"
# ---------------------------------------------------------------------------

STOP_PANEL_STATUS=$(api_post_status "services/portlama-panel/stop")
assert_eq "$STOP_PANEL_STATUS" "400" "Cannot stop portlama-panel (HTTP 400)" || true

# Verify the error message
STOP_PANEL_RESPONSE=$(api_post "services/portlama-panel/stop")
assert_contains "$STOP_PANEL_RESPONSE" "Cannot stop the panel" "Error message explains why panel cannot be stopped" || true

# ---------------------------------------------------------------------------
log_section "Restart portlama-panel is allowed"
# ---------------------------------------------------------------------------

# Restarting the panel may kill the connection mid-response. Tolerate curl errors.
RESTART_PANEL_RESPONSE=$(api_post "services/portlama-panel/restart" 2>/dev/null || echo '{}')
...
if [ "$RESTART_PANEL_OK" = "true" ] || [ -z "$RESTART_PANEL_OK" ] || [ "$RESTART_PANEL_OK" = "null" ]; then
  log_pass "portlama-panel restart request accepted"
else
  log_fail "portlama-panel restart returned unexpected response: $RESTART_PANEL_RESPONSE"
fi
