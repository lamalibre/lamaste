...
# Check if user-access routes are available (requires panel-server >= 0.1.28)
UA_CHECK=$(api_get_status "user-access/grants")
if [ "$UA_CHECK" = "404" ]; then
  log_skip "User-access routes not available (panel-server too old) — skipping grant tests"
